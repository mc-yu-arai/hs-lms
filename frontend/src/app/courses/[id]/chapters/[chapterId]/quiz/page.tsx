...
export function chapterQuizResultStorageKey(courseId: string, chapterId: string) {
  return `chapterQuizResult:${courseId}:${chapterId}`;
}

export default function ChapterQuizTakingPage() {
  ...
  useEffect(() => {
    if (!user) return;
    authFetch<QuizDetail>(`/v1/courses/${courseId}/chapters/${chapterId}/quiz`)
      .then(setQuizDetail)
      .catch((err) => setLoadError(err instanceof ApiError ? err.message : "テストの取得に失敗しました"));
  }, [user, authFetch, courseId, chapterId]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const result = await authFetch<QuizAttemptResult>(`/v1/courses/${courseId}/chapters/${chapterId}/quiz/attempts`, {
        method: "POST",
        body: payload,
      });
      sessionStorage.setItem(chapterQuizResultStorageKey(courseId, chapterId), JSON.stringify({ result, quizDetail }));
      router.push(`/courses/${courseId}/chapters/${chapterId}/quiz/result`);
    } catch (err) {
