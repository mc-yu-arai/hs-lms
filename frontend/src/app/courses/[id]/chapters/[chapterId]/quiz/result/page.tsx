...
import { chapterQuizResultStorageKey } from "../page";
...
export default function ChapterQuizResultPage() {
  ...
  const [stored] = useState<StoredResult | null>(() => {
    ...
    const raw = sessionStorage.getItem(chapterQuizResultStorageKey(courseId, chapterId));
  ...
  useEffect(() => {
    if (!user) return;
    authFetch<{ attempts: QuizAttemptSummary[] }>(`/v1/courses/${courseId}/chapters/${chapterId}/quiz/attempts`)
        ...
        <section className="mb-6 rounded-xl bg-white p-5 text-center shadow-sm sm:p-8">
          {score === null ? (
            ...
            <>
              ...
              {isPassed && <p className="mt-3 text-sm text-green-700">この章の小テストに合格しました。次の章に進めます。</p>}
              {courseCompleted && <p className="mt-1 text-sm text-green-700">全レッスンとテストが完了し、コースを修了しました。</p>}
            </>
          )}

          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <a
              href={`/courses/${courseId}/chapters/${chapterId}/quiz`}
