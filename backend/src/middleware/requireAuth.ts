...
import { findUserById, syncEmail } from "../services/userRepository";
...
export function requireAuth() {
  return (req: Request, _res: Response, next: NextFunction) => {
    void (async () => {
      ...
      let appUser = await findUserById(data.user.id);
      if (!appUser || !appUser.is_active) {
        throw new HttpError(403, "forbidden", "アカウントが無効化されています");
      }

      // Supabase側でメール変更の確認リンクがクリックされていた場合、public.usersを追いつかせる
      if (data.user.email && data.user.email !== appUser.email) {
        await syncEmail(appUser.id, data.user.email);
        appUser = { ...appUser, email: data.user.email };
      }
