...
- [x] 動画アップロードブロック: UI改善。動画アップロード後のユーザーフィードバックで「ファイルを選択」ボタンが分かりづらいと指摘を受け、`CourseForm.tsx`に共通の`FileDropzone`コンポーネントを新規実装。ネイティブの`<input type="file">`は`hidden`にして`<label>`でラップし、破線枠＋アイコン＋「クリックして選択、またはドラッグ＆ドロップ」という見た目にした。`onDragOver`/`onDrop`ハンドラで実際のドラッグ＆ドロップ操作にも対応（見た目だけでなく実際に機能する）。動画（`handleVideoSelected`）とSCORM/LearnWizのzip（`handleZipSelected`）の両方のアップロードUIをこの共通コンポーネントに置き換え、一貫した見た目にした。ブラウザ実機で、動画・SCORM双方のドロップゾーンの表示（破線枠・カーソルpointer等の算出スタイル）と、実際のファイル選択によるアップロード成功（`格納先:`表示への反映）が変更後も機能することを確認済み
- [x] テストCSVインポートブロック: `POST /v1/courses/:id/quiz/import`（admin/super_admin限定、multipart、クエリ`mode=append|replace`必須）・`GET /v1/courses/:id/quiz/import/template`（CSVテンプレートDL）を実装。CSV列構成は`問題文,問題種別(single/multiple),選択肢1〜4,正解`（選択肢は1列目から詰めて2〜4択に対応、正解は単一選択が数字1つ・複数選択がカンマ区切り）。`backend/src/services/quizImportService.ts`（CSVパース・全件事前バリデーション・テンプレート生成）を新規作成し、`quizRepository.ts`に`ensureQuizForCourse`（対象コースにテストが無ければ仮タイトル「修了確認テスト」で自動作成）・`getNextDisplayOrder`・`insertQuestionsIntoQuiz`を追加
- [x] テストCSVインポートブロック: 設計時にユーザーへ「既存設問への追加か全置換か」「テスト未作成コースの扱い」を確認し、**取り込み方法（追加/置換）をユーザーが都度選択できる**・**テスト未作成コースには仮タイトルで自動作成する**という仕様で確定。「1件でもエラーがあれば全件ロールバック」を満たすため事前に全行バリデーションしてから書き込む方式（`userImportService.ts`と同じ考え方）。replaceモードは「新しい設問の挿入が全件成功してから古い設問を削除する」順序にすることで、挿入途中の失敗時に既存データを失わずに済む設計にした（既存の`POST /courses/:id/quiz`のような単純な削除→挿入ではない）
- [x] テストCSVインポートブロック: Jestテスト10件追加（`quizImport.test.ts`。アクセス制御・ファイル未指定・mode未指定/不正値・テスト未作成コースへの自動作成・追加モード・置換モード・行別バリデーションエラーでの全件ロールバック・選択肢の空欄詰め違反、バックエンド合計176件全てパス）。テスト専用フェイクDB（`tests/helpers/fakeSupabase.ts`）に`limit()`が未実装だったため追加した
- [x] テストCSVインポートブロック フロントエンド: `/admin/courses/[id]/quiz`に「CSVでインポート」ボタンを追加し、`QuizImportModal.tsx`（ファイル選択・テンプレートDL・取り込み方法のラジオ選択（追加/置換）・行別バリデーションエラー表示・成功時のインポート件数表示）を新規実装（`ImportUsersModal.tsx`と同様のパターン）。インポート成功時は`loadQuiz()`で設問一覧を再取得しフォームに反映する
- [x] テストCSVインポートブロック: 実データで動作確認済み（ブラウザの自動操作ツールにネイティブファイル選択ダイアログを操作する手段が無いため、ログイン中のブラウザセッションのJS実行コンテキスト内で`File`+`DataTransfer`を生成し隠しファイル入力へ`change`イベントを発火する方式で検証）。検証用に一時的な管理者アカウント`quiz-import-test-admin@example.com`をSupabase上に作成（確認後もアカウント自体は他ブロックの検証用アカウントと同様に残置、動作確認用の一時スクリプトは削除済み）。既存3問のコース「色々試す用」に対し、追加モードで2問追加→5問になることを確認、続けて置換モードで1問のCSVをインポート→既存5問が消え1問のみになり正解の選択肢（2番目）も正しく反映されることを確認、テンプレートDLが200 OKでBOM付きCSVを返すことを確認、問題文空欄・問題種別不正を含むCSVでは400エラーと行別メッセージが表示され既存データが変化しないことを確認
