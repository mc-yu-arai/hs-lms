...
## プロジェクト概要
派遣会社向け学習管理システム「HS-LMS」。**認証・アカウント管理ブロックとコース管理ブロックのAPI実装が完了**。フロントエンド（Next.js、`frontend/`）は認証系（ログイン・2FA・パスワードリセット・プロフィール編集）とコース受講系（コース詳細・レッスン視聴・コース完了）の全10画面を実装済み。
