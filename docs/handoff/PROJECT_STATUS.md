...
## 未着手・進行中
- [ ] フロントエンド: **コース受講画面が未着手**（コース詳細、章・レッスン一覧表示、動画/PDF/テキストのコンテンツ視聴、レッスン進捗更新の操作導線、「受講開始」ボタン）。バックエンドAPI（`GET /courses/:id`, `POST /courses/:id/enroll`, `PUT .../lessons/:lessonId/progress`）は実装済みだが、フロントエンドから呼ぶ画面がまだ無い
